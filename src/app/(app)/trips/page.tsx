...
export default async function TripsPage() {
  ...
  const trips = (memberships ?? [])
    .map((m) => ({ role: m.role, trip: m.trips }))
    .filter((m): m is { role: string; trip: NonNullable<typeof m.trip> } =>
      Boolean(m.trip),
    );

  const tripIds = trips.map((t) => t.trip.id);

  // Aggregate costs + member counts across all trips in two batched queries.
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Meine Reisen & Events</h1>
      ...
      {trips.length === 0 ? (
        <div className="card flex flex-col items-center gap-3 px-6 py-16 text-center">
          ...
          <h2 className="text-lg font-semibold">Noch keine Reise geplant</h2>
          <p className="max-w-sm text-sm text-[var(--muted)]">
            Lege deine erste Reise oder dein erstes Event an und lade Mitreisende
            zur gemeinsamen Planung ein.
          </p>
          <NewTripButton />
        </div>
      ) : (
